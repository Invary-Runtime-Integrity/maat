...
#include <target/invary_runtime_meas_target_type.h>
#include <target/amd_sev_snp_meas_target_type.h>
...
static inline int register_target_types(void)
{
    ...
    if( (ret_val = register_target_type(&invary_runtime_meas_target_type)) != 0) {
        return ret_val;
    }

    if( (ret_val = register_target_type(&amd_sev_snp_meas_target_type)) != 0) {
        return ret_val;
    }
